...
| `media` | `uv run cli.py media <platform>` | `<platform>` (the hub forwards the resolved gate; `download_media.py` also accepts `--min-tier <label>`, `--min-score <n>`, `--top <n>` when run by hand) | Runs the media-download stage: shells to `download_media.py <platform>` (cwd = repo root), which persists the **tier-gated** top clips' videos and thumbnails to `media/<platform>/<content_id>.{mp4,jpg}` so the board can play them and AnalysisEngine can watch them. Selection is a tier/score **gate** applied before the cap, resolved from `niche_config.json`'s `virality.media_filter` (`min_tier` → `--min-score`, optional `max_downloads` → `--top`) by `core/virality.resolve_media_filter`, not a raw top-N by score. No `media_filter` = the pre-gate top-60. | `uv run cli.py media instagram` |
...
| Command | Usage | Args / flags | What it does | Example |
|---|---|---|---|---|
| `run` | `uv run cli.py run <platform>` | `<platform>` | Manual/exhaustive discovery pass. Bootstraps, checks the `discovery_enabled` kill-switch (fail-closed — skips and logs `run.skip` if false), loads niche config and corpus factors, and expands the seed keyword list (Gemini term-expansion runs **only** if `term_expansion_enabled` is true and a Gemini key resolves — it merely *widens* the seed `discovery.keywords`; the default is the seed keywords verbatim). **Actual topic search requires a burner session.** Instagram gates its search surface behind login, so `topsearch` is burner-only: with `guest_only=true` (the default) or no burner present, search is **skipped** with a banner and only the pre-seeded/guest-safe hydration path runs — guest-only discovery is shallow/empty by design. Supply a burner session (`IG_SESSIONID` env or `AutoSearch/session.txt`) **and** set `guest_only=false` to enable real search: it then hydrates profiles, samples reels, scores niche-fit, and posts every passing candidate to `POST /api/discovery/{p}`. Bypasses the weekly plan/cadence but still respects caps, pacing, and the circuit breaker. | `uv run cli.py run instagram` |
