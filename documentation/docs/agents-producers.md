...
**Zero registered proposers is no longer refused — it auto-heals.** When *no* registered producer declares `proposes: true`, the hub falls back to launching the **built-in `similar-content` proposer**, resolved from its sibling directory under the same direct-sibling containment rule (`_builtin_proposer_dir`) and the same argv allowlist (`_validate_render_cmd`) as the registered path, running only the hub-owned default command — never anything from a manifest. That built-in proposer **self-registers on its first run**, so the next launch takes the ordinary registered path and the fallback stops firing. Readiness (`_proposer_status`, which greys the button with its reason) reports **ready** whenever *either* a registered proposer *or* the built-in proposer is available.

**Only the "several proposers" case is still refused.** Two or more producers declaring `proposes: true` is a genuine ambiguity the hub will not guess through unattended, so it is still rejected with a `409` — and that is the reason surfaced as `propose_agent_problem` in `GET /api/cascade`, which is what an operator sees when the last boundary can never fire. (Auto-heal deliberately does not paper over ambiguity — only absence.)
