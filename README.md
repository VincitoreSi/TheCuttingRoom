...
one, prints `HUB_URL=…`, and exports `BACKEND_API` so every agent it spawns follows. Override
with `--port` / `--host` or `HUB_PORT` / `HUB_HOST`. Agents resolve the hub from `BACKEND_API`,
so nothing hardcodes a port. The hub binds loopback only — it is a local tool, not a server.
