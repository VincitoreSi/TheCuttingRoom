#!/usr/bin/env python3
"""engine/logsetup.py — per-run logging, mirroring ReelScraper's core/logsetup.py.

Every AnalysisEngine command calls setup_logging(command, platform) ONCE at startup. It
installs the same two handlers the rest of the pipeline uses (PIPELINE.md §10.1, tier 1
"local, full fidelity"):
...
Modules do `log = logging.getLogger("ae.analyze")` and pass structured fields via
...
concern: HubClient.post_log() POSTs run start/end, per-item done, errors, and eval scores
to `POST /api/logs` with the `run_id` returned here — never every debug line.
"""
